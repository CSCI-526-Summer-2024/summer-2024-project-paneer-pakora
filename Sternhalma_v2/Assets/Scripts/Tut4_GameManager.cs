...
public class Tut4_GameManager : MonoBehaviour
{
    public static int retryCount = 0;
    public static string userId = "user_unique_id";
    public static Tut4_GameManager Instance;
    ...
    public void GenerateHexGrid(float hexSize, Dictionary<Vector3, HexTile> posTile, Dictionary<Vector3, Vector3> posTranslator, HexTile hexPrefab)
    {
        ...
        float hexHeight = hexSize * Mathf.Sqrt(3) + 0.4f;
        ...
        for (float x = -3.0f; x <= 3.0f; x += 1.5f)  //overall range of x
        {
            ...
            else if (x == -3.0f)  //no rotatable tiles here
            {
                float y = -1.0f;
                {
                    float xPos = x * hexWidth;
        ...
        Tut4_GameManager.Instance.ChangeState(GameState.SpawnObjects);
    }
    ...
    public void SpawnObjects(Dictionary<Vector3, BaseUnit> currentStatus, Dictionary<HexTile, BaseUnit> tileToUnit, HashSet<HexTile> isVisited)
    {
        ...
        List<Vector3> paperList = new List<Vector3> { new Vector3(-1.5f, -0.5f), new Vector3(0.0f, -1.0f), new Vector3(0.0f,2.0f) };
        ...
        Tut4_GameManager.Instance.ChangeState(GameState.PlayerTurn);
    }
}
