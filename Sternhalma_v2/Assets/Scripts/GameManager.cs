...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        ChangeState(GameState.GenerateGrid); // Start by generating the grid
        GameManager.Instance.rotateButton.SetActive(false);
    }
    ...
    public void ChangeState(GameState newState)
    {
        ...
        switch (newState)
        {
            ...
            case GameState.Tutorial:
                // Start the tutorial
                Debug.Log("Starting Tutorial, we are in Game Manager");

                TutorialManager.Instance.StartCoroutine(TutorialManager.Instance.StartTutorial());
                break;
            default:
                throw new ArgumentOutOfRangeException(nameof(newState), newState, null);
        }
