...
//        for (float x = -1.5f; x <= 1.5f; x += 1.5f) //overall range of x
//        {
//            if (x == 0)
//            {
//                for (int y = -2; y <= 1; y++)
...
//        for (float x = -1.5f; x <= 1.5f; x += 1.5f)  //range of x
//        {
//            if (x == 0)
//            {
//                for (int y = -2; y <= 1; y++)
...
//        List<Vector3> scissorList = new List<Vector3> { new Vector3(-1.5f, -0.5f) };
...
//        List<Vector3> paperList = new List<Vector3> { new Vector3(-1.5f, 0.5f), new Vector3(0.0f, 1.0f) };
...
public class Level0_GameManager : MonoBehaviour
{
    public static Level0_GameManager Instance;
    public GameState GameState;
    ...
    public void GenerateHexGrid(float hexSize, Dictionary<Vector3, HexTile> posTile, Dictionary<Vector3, Vector3> posTranslator, HexTile hexPrefab)
    {
        ...
        for (float x = -3.0f; x <= 3.0f; x += 1.5f) //overall range of x
        {
            if (x == 0)
            {
                for (int y = -2; y <= 2; y++)   //all 5 hexes on middle row
                {
                    float xPos = x * hexWidth;
                    float yPos = y * hexHeight;
        ...
        GridManager.Instance.posTranslator = posTranslator;
        GridManager.Instance.posTile = posTile;
        Level0_GameManager.Instance.ChangeState(GameState.SpawnObjects);
    }
    ...
    public void SpawnObjects(Dictionary<Vector3, BaseUnit> currentStatus, Dictionary<HexTile, BaseUnit> tileToUnit, HashSet<HexTile> isVisited)
    {
        ...
        List<Vector3> scissorList = new List<Vector3> { new Vector3(-1.5f, -0.5f), new Vector3(1.5f, 0.5f), new Vector3(0.0f, 2.0f) };
        var scissorCount = scissorList.Count;
        ...
        List<Vector3> rockList = new List<Vector3> { new Vector3(-1.5f,-1.5f), new Vector3(1.5f, 1.5f) };
        var rockCount = rockList.Count;
        ...
        //List<Vector3> paperList = new List<Vector3> { new Vector3(1.5f, 0.5f), new Vector3(0, -1), new Vector3(-1.5f, 0.5f) };
        //List<Vector3> paperList = new List<Vector3> { new Vector3(-3.0f, 0f), new Vector3(-1.5f, 1.5f), new Vector3(1.5f, 1.5f), new Vector3(1.5f, -1.5f), new Vector3(3.0f, -1.0f) };
        List<Vector3> paperList = new List<Vector3> { new Vector3(-3.0f, 0.0f), new Vector3(3.0f, 0.0f) };
        var paperCount = paperList.Count;
        ...
        // Change the game state to PlayerTurn after spawning objects
        Level0_GameManager.Instance.ChangeState(GameState.PlayerTurn);
    }
}
