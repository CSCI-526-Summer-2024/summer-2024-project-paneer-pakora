...
public class HexTile : MonoBehaviour
{
    ...
    private void OnMouseDown()
    {
        if (GameManager.Instance.GameState != GameState.PlayerTurn && GameManager.Instance.GameState != GameState.Tutorial)
        {
            return;
        }
        ...
        else if (UnitManager.Instance.currentStatus[this.posEasy] != null && selectedTile == null)
        {
            ...
            // Notify the TutorialManager when the Scissor is selected
            if (this.posEasy == new Vector3(-3.0f, -1.0f) && this.OccupiedUnit != null && this.OccupiedUnit.Faction == Faction.Scissor)
            {
                TutorialManager.Instance.OnScissorSelected();
            }
        ...
        else
        {
            ...
            if ((selectedPos.x == currentPos.x && Mathf.Abs(currentPos.y - selectedPos.y) == 2) ||
                (Mathf.Abs(currentPos.x - selectedPos.x) == 3 && Mathf.Abs(currentPos.y - selectedPos.y) == 1) &&
                UnitManager.Instance.currentStatus[currentPos] == null)
            {
                Vector3 midPos = Vector3.Lerp(selectedPos, currentPos, 0.5f);
                HexTile midTile = GridManager.Instance.GetTileAtPos(GridManager.Instance.GetTranslatedPos(midPos));
                ...
                else if (UnitManager.Instance.currentStatus[selectedTile.posEasy].Faction == Faction.Scissor)
                {
                    if (UnitManager.Instance.currentStatus[midPos] != null && UnitManager.Instance.currentStatus[midPos].Faction == Faction.Paper)
                    {
                        ...
                        // Notify the TutorialManager when the Scissor is moved to (0,0)
                        if (this.posEasy == new Vector3(0, 0))
                        {
                            TutorialManager.Instance.OnScissorMovedToTarget();
                        }
                    }
                    else
                    {
